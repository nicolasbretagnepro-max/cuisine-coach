...
function weeklyProgressCount(){
  var n=0,scoreMap=state.get('lessonScores')||{},notes=state.get('recipeNotes')||{};
  Object.keys(scoreMap).forEach(function(id){if(dateInThisWeek(scoreMap[id]&&scoreMap[id].date))n++;});
  Object.keys(notes).forEach(function(id){if(dateInThisWeek(notes[id]&&notes[id].date))n++;});
  return n;
}
function todayPlan(){
  var next=nextAvailableLesson(),review=lessonToReview(),pending=pendingPracticeItems(3),recipe=(pending[0]&&pending[0].recipe)||relatedRecipeForLesson(next),goal=state.get('weeklyGoal')||2,done=weeklyProgressCount();
  return {lesson:next,review:review,recipe:recipe,pending:pending,weeklyDone:done,weeklyGoal:goal};
}
...
function renderHome(){
  ...
  var plan=todayPlan(),nextRecipe=smartRecipePick(),reviewLesson=lessonToReview(),last=state.get('lastOpened');
  ...
  var weeklyPct=plan.weeklyGoal?Math.min(100,Math.round(plan.weeklyDone/plan.weeklyGoal*100)):0;
  ...
  h+='<div class="hero-sub">Un petit pas concret aujourd’hui</div>';
  ...
  h+='<section class="today-card mt-16" aria-label="Plan du jour">';
  h+='<div class="today-top"><div><div class="today-kicker">Aujourd’hui</div><div class="today-title">Ton entraînement recommandé</div></div>';
  h+='<div class="today-goal"><span>'+plan.weeklyDone+'/'+plan.weeklyGoal+'</span><small>objectif</small></div></div>';
  ...
  if(plan.lesson){
    h+='<a class="today-item" href="#lesson/'+plan.lesson.id+'"><div class="today-icon">📚</div><div class="grow"><div class="today-item-title">Leçon courte</div><div class="today-item-sub">'+esc(plan.lesson.title)+' · '+plan.lesson.duration+' min</div></div><div class="today-arrow">→</div></a>';
  } else {
  ...
  if(plan.recipe){
    h+='<a class="today-item" href="#recipe/'+plan.recipe.id+'"><div class="today-icon">👨‍🍳</div><div class="grow"><div class="today-item-title">Recette pour pratiquer</div><div class="today-item-sub">'+esc(plan.recipe.title)+' · '+((plan.recipe.timePrep||0)+(plan.recipe.timeCook||0))+' min</div></div><div class="today-arrow">→</div></a>';
  }
  h+='</div>';
  h+='<div class="today-note">La progression reste verrouillée : seules les leçons disponibles dans ton parcours sont proposées. Les recettes restent facultatives.</div>';
  h+='</section>';

  if(plan.pending&&plan.pending.length){
    h+='<section class="pending-panel mt-16"><div class="row-sb"><div><div class="t-h3">À pratiquer</div><div class="t-small t-muted mt-4">Tes derniers cours ont ajouté ces exercices. Tu peux cuisiner plus tard sans perdre le fil.</div></div><span class="badge badge-orange">'+plan.pending.length+'</span></div><div class="stack-8 mt-12">';
...
function renderRecipes(filter,search){
  ...
  nh+='<div class="row-sb"><div><div class="t-h4">Que cuisiner aujourd’hui ?</div><div class="t-small t-muted mt-4">Choisis selon ton temps, ton énergie ou la compétence à pratiquer.</div></div></div>';
